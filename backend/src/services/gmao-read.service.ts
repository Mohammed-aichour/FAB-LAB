...
export function resolveMachine(query: string | null | undefined): JsonRecord | null {
  ...
  if (refMatch) {
    ...
    const found = machines.find((m) =>
      normalize(m.reference) === targetRef ||
      normalize(m.id) === targetRef ||
      normalize(m.codeArborescence) === targetRef ||
      (m.codeArborescence && normalize(m.codeArborescence).split('/').some((part: string) => normalize(part) === targetRef))
    );
  ...
  const substringMatches = machines.filter((m) => {
    ...
    if (ref && (nQuery.includes(ref) || (ref.length >= 3 && ref.includes(nQuery)))) return true;
    if (id && (nQuery.includes(id) || id.includes(nQuery))) return true;
    if (name && (nQuery.includes(name) || name.includes(nQuery))) return true;
    if (des && (nQuery.includes(des) || des.includes(nQuery))) return true;
    if (code && (nQuery.includes(code) || code.includes(nQuery))) return true;
    return false;
  });
...
export function matchesMachineRecord(record: JsonRecord, machine: JsonRecord): boolean {
  ...
  if (recEqId) {
    if (mRef && recEqId === mRef) return true;
    if (mId && recEqId === mId) return true;
    if (mCode && mCode.includes(recEqId)) return true;
    if (mRef && recEqId.includes(mRef)) return true;
  }

  if (recEqName) {
    if (mRef && recEqName.includes(mRef)) return true;
    if (mName && (recEqName.includes(mName) || mName.includes(recEqName))) return true;
    if (mDes && (recEqName.includes(mDes) || mDes.includes(recEqName))) return true;
  }
...
export function legacyReadTool(name: string, args: JsonRecord): unknown {
  ...
  if (name === 'list_interventions') {
    ...
    return readEntity<JsonRecord[]>('interventions').filter((record) => {
      if (args.machine) {
        if (resolvedMachine) {
          if (!matchesMachineRecord(record, resolvedMachine)) return false;
        } else {
          if (!matches(record, args.machine, ['equipmentId', 'equipmentName'])) return false;
        }
      }
      if (args.status && !normalize(record.status).includes(normalize(args.status))) return false;
      if (args.technician && !normalize(record.technician).includes(normalize(args.technician))) return false;
      return true;
    });
  }
  if (name === 'list_maintenance') {
    ...
    const resolvedMachine = args.machine ? resolveMachine(args.machine) : null;
    return readEntity<JsonRecord[]>('preventif').filter((record) => {
      if (args.machine) {
        if (resolvedMachine) {
          if (!matchesMachineRecord(record, resolvedMachine)) return false;
        } else {
          if (!matches(record, args.machine, ['fl_id', 'equipement'])) return false;
        }
      }
      if (args.overdueOnly && (!record.prochaineEcheance || record.prochaineEcheance >= today || record.statut === 'Terminé')) return false;
      return true;
    });
  }
