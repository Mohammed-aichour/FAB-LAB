...
const schemas = {
  ...
  create_intervention: z.object({
    machine: z.string().min(1), description: z.string().min(1), maintenanceType: z.enum(['Corrective', 'Préventive']),
    plannedDate: z.iso.date(), priority: z.enum(['A', 'B', 'C']), technician: z.string().optional(),
  }),
...
function findUnique(records: RecordValue[], query: string, fields: string[], label: string): RecordValue {
  ...
  if (partial.length > 1 || exact.length > 1) throw new Error(`${label} ambigu : précisez la référence ou l'identifiant.`);
  throw new Error(`${label} introuvable dans les données réelles.`);
}

const findMachine = (query: string) => findUnique(readEntity<RecordValue[]>('machines'), query, ['id', 'reference', 'name', 'designation'], 'Machine');
