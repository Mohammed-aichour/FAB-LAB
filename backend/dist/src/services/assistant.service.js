...
const assistant_actions_service_1 = require("./assistant-actions.service");
const gmao_read_service_1 = require("./gmao-read.service");
const tools_1 = require("../agent/tools");
const extra_tools_1 = require("../agent/extra-tools");
const model_1 = require("../agent/model");
const instructions_1 = require("../agent/instructions");
exports.allTools = [...tools_1.tools, ...extra_tools_1.extraTools];
...
function selectTools(messages) {
    const lastUserMessage = [...messages].reverse().find(message => message.role === 'user')?.content || '';
    const mutation = actionPattern.test(lastUserMessage);
    const selected = exports.allTools.filter(tool => mutation === assistant_actions_service_1.MUTATION_TOOL_NAMES.has(tool.name));
    return { mutation, tools: selected };
}
function readTool(name, raw) {
    const schema = extra_tools_1.readSchemas[name];
    if (!schema)
        throw new Error('Outil de lecture non autorisé.');
    const args = schema.parse(raw);
    const result = tools_1.tools.some(t => t.name === name) ? (0, gmao_read_service_1.legacyReadTool)(name, args) : (0, extra_tools_1.extraRead)(name, args);
    return Array.isArray(result) ? { total: result.length, items: result.slice(0, 100), truncated: result.length > 100 } : result;
}
async function runAssistant(messages, user, model = model_1.createResponse) {
    if (user.role !== 'Superviseur' || user.status !== 'Actif')
        throw new Error('Accès réservé au superviseur.');
    ...
    for (let turn = 0; turn < 8; turn++) {
        ...
        if (response.status === 'incomplete' && !hasUsableOutput) {
            console.error('[Assistant incomplete]', { details: response.incomplete_details, outputTypes: (response.output || []).map((item) => item.type), usage: response.usage });
            throw new Error('Le modèle n’a pas terminé sa réponse. Réessayez avec une question plus courte.');
        }
        const calls = (response.output || []).filter((item) => item.type === 'function_call');
        if (!calls.length) {
            const message = (response.output || []).filter((item) => item.type === 'message').flatMap((item) => item.content || []).filter((part) => part.type === 'output_text').map((part) => part.text).join('\n').trim();
        ...
        for (const call of calls) {
            let result;
            try {
                if (!selected.tools.some(t => t.name === call.name))
                    throw new Error('Outil non autorisé pour cette demande.');
                ...
                if (assistant_actions_service_1.MUTATION_TOOL_NAMES.has(call.name)) {
                    ...
                    const pendingAction = (0, assistant_actions_service_1.createPendingAction)(call.name, clean, user);
