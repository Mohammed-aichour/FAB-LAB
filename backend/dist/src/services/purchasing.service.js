...
exports.componentSupplier = componentSupplier;
exports.buildPurchase = buildPurchase;
const crypto_1 = __importDefault(require("crypto"));
const zod_1 = require("zod");
const json_store_1 = require("./json-store");
const normalize = (v) => String(v ?? '').trim().toLocaleLowerCase('fr');
function unique(records, query, fields, label) {
    if (!query.trim())
        throw new Error(`${label} : référence requise.`);
    ...
    if (found.length !== 1)
        throw new Error(found.length ? `${label} ambigu : précisez un identifiant exact.` : `${label} introuvable dans les données actuelles.`);
    return found[0];
}
function suppliers() {
    return (0, json_store_1.readEntity)('suppliers').filter(s => s.code !== 'Code').map(s => ({
        ...s, domain: s.domain ?? s.domaine ?? '', email: s.email || (zod_1.z.email().safeParse(s.telEmail).success ? s.telEmail : ''),
...
function componentSupplier(query) {
    const component = unique((0, json_store_1.readEntity)('stock'), query, ['id', 'reference', 'name'], 'Composant');
...
exports.purchaseSchema = zod_1.z.object({ component: zod_1.z.string().min(1).max(200), quantity: zod_1.z.number().positive().max(1000000), kind: zod_1.z.enum(['commande', 'devis', 'reapprovisionnement']) }).strict();
function buildPurchase(raw) {
    const input = exports.purchaseSchema.parse(raw);
    const { component, supplier } = componentSupplier(input.component);
    if (!supplier)
        throw new Error('Aucun fournisseur associé identifiable. Complétez la fiche composant avant de préparer une commande.');
    if (supplier.status === 'Inactif')
        throw new Error('Le fournisseur est inactif.');
    ...
    return { id: crypto_1.default.randomUUID(), reference: `CDE-${crypto_1.default.randomUUID().slice(0, 8)}`, createdAt: new Date().toISOString(),
