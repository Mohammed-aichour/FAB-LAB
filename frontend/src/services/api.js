    ...
    try {
        ...
        if (contentType.includes('application/json')) {
            ...
            if (response.status === 401)
                window.dispatchEvent(new Event('gmao_session_expired'));
            throw new Error(data.error || 'Erreur serveur.');
        }
        return await handleStaticFallback(path, body);
    }
    catch (err) {
...
async function handleStaticFallback(path, body) {
    ...
    if (path === '/auth/login') {
        ...
        if (foundUser && (password === 'password123' || !password)) {
            return { token: 'static_session_token_' + Date.now(), user: foundUser };
        }
    ...
    if (path.startsWith('/db/')) {
        ...
        if (storageKey) {
            if (body !== undefined) {
                ...
                return { success: true, count: Array.isArray(body) ? body.length : 1, revision: 'static_rev' };
            }
            else {
